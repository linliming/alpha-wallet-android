...
internal constructor(
    ...
    fun handleQRCode(
        ...
        try {
            ...
            parser.parse(qrCode)?.let { qrResult ->
                when (qrResult.type) {
                    ...
                    EIP681Type.PAYMENT, EIP681Type.TRANSFER -> {
                        props.put(QrScanResultType.KEY, QrScanResultType.ADDRESS_OR_EIP_681.value)
                        track(Analytics.Action.SCAN_QR_CODE_SUCCESS, props)
    ...
    private fun doShowWhatsNewDialog(
        context: Context,
        releases: MutableList<GitHubRelease>,
    ) {
...
//                        assetDefinitionService.resetAttributes(td)
     ...
     */
    private suspend fun getGitHubReleases(request: Request): MutableList<GitHubRelease> {
                ...
                httpClient
                    ...
                    .use { response ->
                        if (response.code / 100 == 2) {
                            return@withContext Gson().fromJson<MutableList<GitHubRelease>>(
                                response.body?.string(),
                                object : TypeToken<MutableList<GitHubRelease?>?>() {
                                }.type,
                            )
                        } else {
                            return@withContext mutableListOf<GitHubRelease>()
                        }
                    }
            } catch (e: Exception) {
                Timber.e(e)
